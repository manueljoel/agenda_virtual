...
class Classe
{
    ...
    function constructArg($idclasse, $descricao, $idciclo)
    {
    ...
    {
        $sql = "select idclasse, descricao, idciclo from classe where idclasse = $id";
        $result = $this->pdo->conect()->query($sql);
        return $result->fetchAll();
    }
    ...
    function inserirclasse()
    {
        ...
        $sql = "INSERT INTO classe (descricao, idciclo) VALUES (:descricao, :idciclo)";
    ...
    function editarClasse()
    {
        ...
        $sql = "UPDATE classe SET descricao=:descricao, idciclo=:idciclo WHERE idclasse=:idclasse";
        $stmt = $this->pdo->conect()->prepare($sql);
        print_r($data);
        return $stmt->execute($data);
    }
